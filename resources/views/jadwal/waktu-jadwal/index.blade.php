...
<div class="container mt-5">
    ...
    <div class="card">
        <div class="card-header">
            <h1 class="text-center">TABEL WAKTU JADWAL</h1>

            <div class="text-right">
                <a class="btn btn-primary" href="/waktu-jadwal/create-index/{{$jadwal_id}}">
            ...
            <div class="table-responsive">
                <table class="table table-striped" id="example2">
                    ...
                    <tbody class="text-center">
                        ...
                        $no = 1;
                        @endphp
                        @foreach ($data_waktu_jadwal as $dtwj )
                        <tr>
                            <td>{{$no++}}</td>
                            <td>{{$dtwj->jadwal->guru->user->name}}</td>
                            <td>{{$dtwj->jadwal->kelas->nama}}</td>
                            <td>{{$dtwj->jadwal->mata_pelajaran->mata_pelajaran}}</td>
                            <td>{{$dtwj->hari}}</td>
                            <td>{{$dtwj->jam_awal}}</td>
                            <td>{{$dtwj->jam_akhir}}</td>
                            <td>

                                <a href="/waktu-jadwal/edit-index/{{$dtwj->id}}" class="btn btn-sm btn-warning"><i class="fas fa-edit text-white"></i></a>

                                <a href="/waktu-jadwal/delete/{{$dtwj->id}}" class="btn btn-sm btn-danger"><i class="fas fa-trash"></i></a>
